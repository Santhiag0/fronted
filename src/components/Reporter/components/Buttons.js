...
function Buttons({
  ...
  const runQuery = async () => {
    ...
    console.log("llega");
    try {
      const { tableHeaders, tableRows } = await GetTableInfo(defaults);
  ...
  return (
    <div className="flex">
      <div className="p-2">
        <button
          onClick={reset}
          className="flex mx-auto text-white bg-black border-0 py-2 h-11 px-4 focus:outline-none hover:bg-slate-500 rounded text-lg justify-center items-center"
        >
          <svg
      ...
      <div className="p-2">
        ...
        >
          ...
          <div className="font-bold font-mono">EJECUTAR SENTENCIA</div>
        </button>
      </div>
    </div>
  );
